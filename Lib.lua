...
local Themes = {
    ...
    ["rainbow"]      = "rainbow", -- Специальная переливающаяся тема
    ["радуга"]       = "rainbow",
    ["purple"]       = Color3.fromRGB(192, 132, 252),
    ["red"]          = Color3.fromRGB(255, 85, 85),
...
local function GetColor(input, fallbackThemeColor)
    ...
    -- Если fallback это радуга, возвращаем белый для текста, градиент сделает свое дело
...
-- Универсальная загрузка ассетов
...
function LuminorLib:CreateWindow(titleText, uiName, watermarkText, themeName, bgName)
    ...
    -- ГЛАВНАЯ ПАНЕЛЬ (Где лежат кнопки)
    ...
    -- === ЛОГИКА ПЕРЕЛИВАНИЯ ОБВОДКИ (АНИМАЦИЯ) ===
    ...
    if isClassic then
        ...
        if isRainbow then
            ...
            -- Перелив выбранного цвета (от яркого к темному)
...
-- Попробуй темы: "red", "rainbow", "classic"
local Window = LuminorLib:CreateWindow("Luminor", "LuminorUI", "By DADILK", "red", "none")

local Tab = Window:CreateTab("Главная", "")

Tab:CreateMarkdown("<b>Смотри на обводку панели!</b>\nЕсли тема не классическая, она плавно переливается градиентом.")

Tab:CreateToggle("Авто-Фарм", false, nil, "", function(state)
    print("Авто-фарм:", state)
end)

local scrollBox = Tab:CreateScrollingBox(80)
local logText = Instance.new("TextLabel")
logText.Size = UDim2.new(1, 0, 0, 25)
logText.BackgroundTransparency = 1
logText.TextColor3 = Color3.fromRGB(200, 200, 200)
logText.Text = "> Анимация панели восстановлена."
logText.TextXAlignment = Enum.TextXAlignment.Left
logText.Parent = scrollBox
