...
function LuminorLib:CreateWindow(titleText, uiName, watermarkText, themeName, bgName)
    ...
    if hasBackground then
        ...
        task.spawn(function()
            ...
            if asset and asset ~= "" then
                pcall(function()
                    if isVideo then BackgroundContainer.Video = asset else BackgroundContainer.Image = asset end
                end)
            end
        end)
    end
    ...
    task.spawn(function()
        ...
        KeyInput.PlaceholderText = "Enter Platoboost Key..."
        ...
        -- СЮДА ВСТАВЛЯЕШЬ ССЫЛКУ НА СВОЙ ПЛАТОБУСТ
        ...
        GetKeyBtn.MouseButton1Click:Connect(function()
            if setclipboard then
                ...
                KeyStatus.Text = "Your executor doesn't support setclipboard!"
